...
namespace BGAL
{
  ...
  int _LBFGS::minimize(fun &f, Eigen::VectorXd &iterX)
  {
    ...
    double grad_norm = gradient.norm();
      ...
      {
        std::cout << 0 << "\t" << 0 << "\t" << elapsed_seconds() << "\t" << grad_norm << "\t"
                  << fval << std::endl;
      }
      if (_parameter.iteration_callback)
      {
        _parameter.iteration_callback(0, 0, elapsed_seconds(), grad_norm, fval);
      }
    ...
    while (1)
    {
      ...
      if (_parameter.max_time > 0 && _parameter.max_time < elapsed_seconds() * 1000.0)
      {
        if (_parameter.is_show)
        {
      ...
      if (_parameter.is_show)
      {
        std::cout << k << "\t" << l << "\t" << elapsed_seconds() << "\t" << grad_norm
                  << "\t" << fval << std::endl;
      }
      if (_parameter.iteration_callback)
      {
        _parameter.iteration_callback(k, l, elapsed_seconds(), grad_norm, fval);
      }
      s[cursor] = iterX - oldX;
